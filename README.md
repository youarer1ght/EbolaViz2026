...
```
展示层:  5 个视图 + 热力图分栏 (overview choropleth + province zoom detail + zone scatter)
...
│   ├── views/
│   │   ├── heatmapView.js       # ① 时空热力图 (双 series 边框叠加 + 分栏布局 + 省级放大 + 卫生区散点选择)
...
- **热力图 (分栏布局 + 省级放大)**：
  1. 左侧总览：双 Series 叠加 — 底层统一细边框 + choropleth 着色，顶层透明填充 + 粗边框（解决 ECharts 单 series 共享边缘边框裁剪问题）
  2. 右侧省级详情：点击省份 → 显示该省放大 GeoJSON 地图 + 卫生区散点标记（可独立点击切换选中）
  3. 部分选中：当省份内仅部分卫生区被选中时，显示虚线边框
  4. 省份↔卫生区双向映射：`provinceToZones` / `zoneToProvince` 查找表，聚合 ADM1 与子区域
- **平行坐标**：5 个数值维度 → 按卫生区死亡率着色 (绿→红渐变) → 选中线宽 3.5px / 未选中 1px + 0.45 透明度 → brushSelected → `SET_PARALLEL_AXES_FILTER` → 联动所有视图
- **详情面板**：读取 state → 计算选定区域 summary → 4 张统计卡片 (确诊/死亡/CFR/疫区) + 区域排序表 + 已选区域人口医疗概况
...
**测试 B4：热力图 Choropleth + 省级放大 + 卫生区选择**

1. 打开 `http://localhost:<端口>`
2. 确认热力图显示的是**彩色填充地图**（不是散点气泡），说明 GeoJSON 加载成功
3. 确认热力图区域分为左右两栏：左侧为总览图，右侧显示 "👆 点击左侧地图省份 查看卫生区详情"
4. 点击 "Ituri" 省份 → 省份金色高亮 + **完整粗边框**环绕（共享边缘不再消失）+ 右侧面板显示 Ituri 放大图 + 4 个卫生区彩色散点 (Mongbalu, Nyakunde, Rwampara, Bunia)
5. 在右侧面板点击 "Mongbalu" 散点 → Mongbalu 取消选中（散点颜色改变），其他 3 个仍选中，其他视图联动更新
6. 再次点击 "Mongbalu" → 恢复选中
7. 全部取消 Ituri 下 4 个卫生区 → 右侧面板自动收起，显示占位提示
8. 悬浮 "Nord-Kivu" → 省份变浅金色 + 阴影 + 中等粗边框
...
| # | 操作 | 期望联动效果 | 涉及视图 | 通过 |
|---|------|-------------|:------:|:----:|
| I1 | 热力图 **点击** Ituri 省 | 省份金色粗边框完整环绕；右侧面板显示 Ituri 放大图 + 4 个卫生区散点；时序图仅显示 4 个卫生区曲线；详情面板更新 | ①②⑤ | ☐ |
| I2 | 右侧面板 **点击** "Mongbalu" 散点 | 仅 Mongbalu 切换选中状态；左侧地图 Ituri 变为虚线边框 (部分选中)；其他视图仅显示剩余选中区域 | ①②③⑤ | ☐ |
| I3 | 热力图 **再次点击** Ituri 省 | 取消所有选中；右侧面板收起；所有区域恢复显示 | ①②⑤ | ☐ |
| I4 | 时序图 dataZoom **刷选** 5/18-5/24 | 热力图数据聚合到该时段；平行坐标过滤；详情更新 | ①②③⑤ | ☐ |
| I5 | 热力图 **悬浮** Nord-Kivu | 省份浅金色 + 阴影 + 中等粗边框；时序图对应曲线加粗 → 鼠标移出恢复 | ①② | ☐ |
| I6 | 按 `Space` 开始播放 | 所有视图逐日期更新；日期显示更新；5 帧后暂停正常 | ①②③④⑤ | ☐ |
| I7 | 按 `R` 重置 | 所有筛选清除；全部视图恢复初始状态 | ①②③④⑤ | ☐ |
| I8 | 平行坐标 **brush 刷选** 高死亡率区域 | 热力图仅显示对应省份；时序图过滤；详情更新 | ①②③⑤ | ☐ |
| I9 | 政策轴 **点击** P001 (首个响应) | 热力图添加 markLine；详情面板显示政策 | ①④⑤ | ☐ |
| I10 | 选中 0 个区域 | 全部数据正常显示 (不报错不崩溃) | 全部 | ☐ |
| I11 | 连续快速点击（压力测试） | 无卡顿、无报错、无状态不一致 | 全部 | ☐ |
| I12 | 调整浏览器窗口大小 | 所有视图自适应缩放（含热力图分栏），无重叠空白 | 全部 | ☐ |
